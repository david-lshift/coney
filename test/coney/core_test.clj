...
(fact "Get names works"
      (with-fake-http ["http://localhost:15672/api/users" "[{\"name\":\"bar\",\"password_hash\":\"foo\",\"tags\":\"\"}]"]
        (core/get-names-from-api "users")) => {:bar {:name "bar", :password_hash "foo", :tags ""}})

(defn no-vhost
  ([] (no-vhost "localhost"))
  ([hostname]
   (fn [request]
     (contains? #{(str "http://" hostname ":15672/api/users")
...
(fact "main"
      (with-state-changes [(around :facts (with-redefs
        ...
        (fact "Argument file must exist" (with-out-str (with-redefs [slurp (fn [& _] (throw (java.io.FileNotFoundException.)))
                                                         ...
                                                         (core/-main "Foo"))) => (contains "No such file 'Foo'"))
                    ...
                    (with-fake-http [(no-vhost) "{}"
                      ...
                      (run-with-file "{\"bindings\": [{
                           \"destination\": \"orders.hoxton\",
                             ...
                             \"source\": \"orders\",
                             \"vhost\": \"some-vhost\"}]}"
                                     (core/-main "--filetype" "json" "Foo"))) => (contains "missing/wrong bindings for 'orders.hoxton-queue-{}--orders'"))) ; end "Does export-style JSON"
